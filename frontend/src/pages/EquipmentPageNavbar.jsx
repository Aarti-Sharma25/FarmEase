...
const FarmSpaceNavbar = ({ searchQuery, setSearchQuery, onSearch }) => {
  const { logout, user } = useAuthStore();
  const navigate = useNavigate();
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [searchFocused, setSearchFocused] = useState(false);
  ...
  const clearSearch = () => {
    setSearchQuery('');
    setSearchFocused(false);
    onSearch(''); // empty query => go back to the full (unfiltered) listing
  };

  // Search button click / Enter key press — this is the ONLY place that
  // actually triggers the semantic search API call.
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-gradient-to-r from-green-700 to-emerald-800 py-3 px-4 flex items-center z-50 shadow-lg">
      ...
      >
        <div className="relative">
          {/* Search icon is now a real submit button, not just decoration */}
          <button
            type="submit"
            aria-label="Search"
