...
import { useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { fetchRentals, searchRentals } from '../store/rentalSlice';
import EquipmentPageNavbar from './EquipmentPageNavbar';
import { FaHeart, FaHeartBroken, FaTractor, FaSearch, FaMapMarkerAlt } from 'react-icons/fa';
...
const BrowseEquipmentPage = () => {
  ...
  // Semantic search state — separate from the normal "browse all" state above,
  // because search results come from a different API call (embeddings + cosine
  // similarity on the backend) and shouldn't wipe out the full listing.
  const searchResults = useSelector((state) => state.rentals.searchResults);
  const searchStatus = useSelector((state) => state.rentals.searchStatus); // 'idle' | 'loading' | 'succeeded' | 'failed'
  const searchError = useSelector((state) => state.rentals.error);
  ...
  const [searchQuery, setSearchQuery] = useState('');
  // Tracks whether the user has actually submitted a search (Enter/click),
  // as opposed to just typing. While false, we show the normal browse grid.
  const [isSearchActive, setIsSearchActive] = useState(false);
  const [likedEquipment, setLikedEquipment] = useState(new Set());
  const [categoryFilter, setCategoryFilter] = useState('all');
  const navigate = useNavigate();

  // Called only when the Search button is clicked / Enter is pressed
  // (passed down as the onSearch prop to EquipmentPageNavbar).
  const handleSearch = (query) => {
    const trimmed = query.trim();

    if (!trimmed) {
      // Empty/invalid query -> just go back to browsing everything,
      // no point calling the API with nothing to search for.
      setIsSearchActive(false);
      return;
    }

    setIsSearchActive(true);
    dispatch(searchRentals(trimmed));
  };

  // Which list are we actually showing?
  // - Search active -> semantic search results (still respects category filter)
  // - Otherwise -> the full listing (still respects category filter)
  const baseList = isSearchActive ? searchResults : rentals;
  const filteredEquipment = baseList.filter(item => {
    const matchesCategory = categoryFilter === 'all' || item?.category === categoryFilter;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pt-24">
          ...
          <div className="mb-6 flex items-center text-gray-600">
            ...
            {searchStatus === 'loading' ? (
              ...
              <p className="text-red-600">
                Search failed{searchError ? `: ${searchError}` : ''}. Please try again.
              </p>
            ) : (
              <p>
                Search results for: <span className="font-semibold">"{searchQuery}"</span>
                {' '}({filteredEquipment.length} found)
              </p>
            )}
          </div>
        )}
        ...
        {isSearchActive && searchStatus === 'loading' ? (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center py-12">
            ...
            <p className="text-gray-500">Finding the most relevant equipment...</p>
          </div>
        ) : filteredEquipment.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center py-12">
            <div className="bg-gray-200 border-2 border-dashed rounded-xl w-16 h-16 flex items-center justify-center mb-4">
            ...
            <p className="text-gray-500 max-w-md">
              {isSearchActive
                ? "We couldn't find anything relevant to your search. Try describing what you need differently."
                : rentals.length === 0
                ? 'It seems there are no equipment listings at the moment. Check back later or list your own equipment.'
                : 'No equipment matches this category. Try a different filter.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
